...
請生成三個不同方向的研究題目：
1. 理論導向：基於文獻中的設計理論框架，探討設計研究的理論基礎
2. 實務導向：針對文獻中發現的設計實務問題，提出具體的解決方案
3. 整合導向：結合設計理論與實務的創新角度，探索設計研究的新可能性
...
def generate_full_content(research_topic, research_content, literature_summary, selected_title):
    ...
    try:
        # 構建提示詞
        prompt = f"""你是一位具有豐富設計研究與實務經驗的學者，請根據以下資訊，以自然且專業的學術論述方式，生成一份完整的研究目的和參考文獻：
